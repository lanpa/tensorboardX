...
def convert_to_HWC(tensor, input_format):  # tensor: numpy array
    import numpy as np
    assert(len(set(input_format)) == len(input_format)), "You can not use the same dimension shordhand twice. input_format: {input_format}".format(input_format)
    assert(len(tensor.shape) == len(input_format)), "size of input tensor and input format are different. shape: {}, input_format: {}".format(tensor.shape, input_format)
